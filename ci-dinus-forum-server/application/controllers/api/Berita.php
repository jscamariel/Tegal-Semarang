<?php

use Restserver\Libraries\REST_Controller;
defined('BASEPATH') OR exit('No direct script access allowed');

require APPPATH . 'libraries/REST_Controller.php';
require APPPATH . 'libraries/Format.php';

class Berita extends REST_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Berita_model');
    }

    public function index_get()
    {       
        $id_thread = $this->get('id_thread');
        if($id_thread===null){
            $berita = $this->Berita_model->getAllBerita();
        }else{
            $berita = $this->Berita_model->getAllBerita($id_thread);
        }
        

        if($berita){
            $this->response([
                'status' => true,
                'data' => $berita
            ], REST_Controller::HTTP_OK);
        }else{
            $this->response([
                'status' => false,
                'message' => 'id not found'
            ], REST_Controller::HTTP_NOT_FOUND);
        }
        
    }

    public function index_delete(){
        $id_thread = $this->delete('id_thread');
        if($id_thread===null){
            $this->response([
                'status' => false,
                'message' => 'add some id'
            ], REST_Controller::HTTP_BAD_REQUEST);
        }else{
            if($this->Berita_model->deleteBerita($id_thread) > 0){
                $this->response([
                    'status' => true,
                    'id_thread' => $id_thread,
                    'message' => 'Berhasil Dihapus'
                ], REST_Controller::HTTP_NO_CONTENT);
            }else{
                $this->response([
                    'status' => false,
                    'message' => 'add some id'
                ], REST_Controller::HTTP_BAD_REQUEST);
            }
        }
    }

    public function index_post(){
        $data = [
            'id_thread' => $this->post('id_thread'), // Automatically generated by the model
            'username' => $this->post('username'),
            'nama_thread' => $this->post('nama_thread'),
            'isi' => $this->post('isi'),
            'timestamp' => $this->post('timestamp')
        ];

        if($this->Berita_model->createBerita($data) > 0){
            $this->response([
                'status' => true,
                'message' => 'Data Baru Berhasil Dibuat'
            ], REST_Controller::HTTP_CREATED);
        }else{
            $this->response([
                'status' => false,
                'message' => 'failed to create'
            ], REST_Controller::HTTP_BAD_REQUEST);
        }
    }

    public function index_put(){
        $id_thread = $this->put('id_thread');
        
        $data = [
            'id_thread' => $this->put('id_thread'), // Automatically generated by the model
            'username' => $this->put('username'),
            'nama_thread' => $this->put('nama_thread'),
            'isi' => $this->put('isi'),
            'timestamp' => $this->put('timestamp')
        ];

        if($this->Berita_model->updateBerita($data,$id_thread) > 0){
            $this->response([
                'status' => true,
                'message' => 'Data Berhasil Diupdate'
            ], REST_Controller::HTTP_NO_CONTENT);
        }else{
            $this->response([
                'status' => false,
                'message' => 'failed to update'
            ], REST_Controller::HTTP_BAD_REQUEST);
        }
    }
}