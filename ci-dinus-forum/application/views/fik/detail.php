                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <button class="btn button liked likebtn" data="<?= $fik['id_thread'] ?>" title="Like"> Like</button>

                        </div>
                        <div class="col-sm-6">
                            <button class="btn button  disliked dislikebtn" data="<?= $fik['id_thread'] ?>" title="Dislike">Dislike</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $('.liked').click(function() {
        ...
        if (btn.hasClass('unlikebtn')) {
            $.ajax({
                url: '<?= base_url('fik/unlike/') ?>' + id_thread,
        ...
        if (btn.hasClass('likebtn')) {
            $.ajax({
                url: '<?= base_url('fik/like/') ?>' + id_thread,
                success: function(result) {
    ...
    $('.disliked').click(function() {
        ...
        if (btn.hasClass('undislikebtn')) {
            $.ajax({
                url: '<?= base_url('fik/undislike/') ?>' + id_thread,
                success: function(result) {
        ...
        if (btn.hasClass('dislikebtn')) {
            $.ajax({
                url: '<?= base_url('fik/dislike/') ?>' + id_thread,
                success: function(result) {
