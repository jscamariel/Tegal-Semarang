<div class="container-content">
    <row class="mt-3">
        <div class="col-md-6">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <button class="btn button liked likebtn" data="<?= $fotografi['id_thread'] ?>" title="Like"> Like </button>

                        </div>
                        <div class="col-sm-6">
                            <button class="btn button  disliked dislikebtn" data="<?= $fotografi['id_thread'] ?>" title="Dislike">Dislike</button>
                        </div>
                    </div>
            ...
            <form method="POST" action="<?= base_url('fotografi/kirimKomen/') . $fotografi['id_thread']; ?>">
            ...
            <div class="card mt-1">
                <div class="card-body">
                    ...
                    <?php foreach ($komentar as $komen) : ?>
                        ...
                        <?php if ($komen['id_kategori'] != $fotografi['id_kategori']) : ?>
                        <?php else : ?>
                            <?php if ($komen['id_thread'] == $fotografi['id_thread']) : ?>
                                ...
                                <?php else : ?>
                                    <a href="<?= base_url(); ?>fotografi/hapusKomen/<?= $komen['id_komentar']; ?>" class="badge badge-danger float-right" onclick="return confirm('Yakin?');">Hapus</a>

                                    <a href="<?= base_url(); ?>fotografi/ubah/<?= $komen['id_komentar']; ?>" class="badge badge-warning float-right">Ubah</a>
                                <?php endif; ?>
        ...
        <a href="<?= base_url(); ?>fotografi" class="btn btn-primary mt-3 ml-3">Kembali</a>
    </row>
</div>
...
<script>
    $('.liked').click(function() {
        ...
        if (btn.hasClass('unlikebtn')) {
            $.ajax({
                url: '<?= base_url('fotografi/unlike/') ?>' + id_thread,
        ...
        if (btn.hasClass('likebtn')) {
            $.ajax({
                url: '<?= base_url('fotografi/like/') ?>' + id_thread,
                success: function(result) {
    ...
    $('.disliked').click(function() {
        ...
        if (btn.hasClass('undislikebtn')) {
            $.ajax({
                url: '<?= base_url('fotografi/undislike/') ?>' + id_thread,
                success: function(result) {
        ...
        if (btn.hasClass('dislikebtn')) {
            $.ajax({
                url: '<?= base_url('fotografi/dislike/') ?>' + id_thread,
                success: function(result) {
