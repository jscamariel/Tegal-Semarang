<div class="container-content">
    <row class="mt-3">
        <div class="col-md-6">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <button class="btn button liked likebtn" data="<?= $game['id_thread'] ?>" title="Like"> Like </button>

                        </div>
                        <div class="col-sm-6">
                            <button class="btn button  disliked dislikebtn" data="<?= $game['id_thread'] ?>" title="Dislike">Dislike</button>
                        </div>
                    </div>
            ...
            <form method="POST" action="<?= base_url('game/kirimKomen/') . $game['id_thread']; ?>">
            ...
            <div class="card mt-1">
                <div class="card-body">
                    ...
                    <?php foreach ($komentar as $komen) : ?>
                        ...
                        <?php if ($komen['id_kategori'] != $game['id_kategori']) : ?>
                        <?php else : ?>
                            <?php if ($komen['id_thread'] == $game['id_thread']) : ?>
                                ...
                                <?php else : ?>
                                    <a href="<?= base_url(); ?>game/hapusKomen/<?= $komen['id_komentar']; ?>" class="badge badge-danger float-right" onclick="return confirm('Yakin?');">Hapus</a>

                                    <a href="<?= base_url(); ?>game/ubah/<?= $komen['id_komentar']; ?>" class="badge badge-warning float-right">Ubah</a>
                                <?php endif; ?>
        ...
        <a href="<?= base_url(); ?>game" class="btn btn-primary mt-3 ml-3">Kembali</a>
    </row>
</div>
...
<script>
    $('.liked').click(function() {
        ...
        if (btn.hasClass('unlikebtn')) {
            $.ajax({
                url: '<?= base_url('game/unlike/') ?>' + id_thread,
        ...
        if (btn.hasClass('likebtn')) {
            $.ajax({
                url: '<?= base_url('game/like/') ?>' + id_thread,
                success: function(result) {
    ...
    $('.disliked').click(function() {
        ...
        if (btn.hasClass('undislikebtn')) {
            $.ajax({
                url: '<?= base_url('game/undislike/') ?>' + id_thread,
                success: function(result) {
        ...
        if (btn.hasClass('dislikebtn')) {
            $.ajax({
                url: '<?= base_url('game/dislike/') ?>' + id_thread,
                success: function(result) {
