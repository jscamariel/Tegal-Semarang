<div class="container-content">
    <row class="mt-3">
        <div class="col-md-6">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <button class="btn button liked likebtn" data="<?= $olahraga['id_thread'] ?>" title="Like"> Like </button>

                        </div>
                        <div class="col-sm-6">
                            <button class="btn button  disliked dislikebtn" data="<?= $olahraga['id_thread'] ?>" title="Dislike">Dislike</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="<?= base_url('olahraga/kirimKomen/') . $olahraga['id_thread']; ?>">
        ...
        <a href="<?= base_url(); ?>olahraga" class="btn btn-primary ml-3 mt-3">Kembali</a>
    </row>
</div>
...
<script>
    $('.liked').click(function() {
        ...
        if (btn.hasClass('unlikebtn')) {
            $.ajax({
                url: '<?= base_url('olahraga/unlike/') ?>' + id_thread,
        ...
        if (btn.hasClass('likebtn')) {
            $.ajax({
                url: '<?= base_url('olahraga/like/') ?>' + id_thread,
                success: function(result) {
    ...
    $('.disliked').click(function() {
        ...
        if (btn.hasClass('undislikebtn')) {
            $.ajax({
                url: '<?= base_url('olahraga/undislike/') ?>' + id_thread,
                success: function(result) {
        ...
        if (btn.hasClass('dislikebtn')) {
            $.ajax({
                url: '<?= base_url('olahraga/dislike/') ?>' + id_thread,
                success: function(result) {
