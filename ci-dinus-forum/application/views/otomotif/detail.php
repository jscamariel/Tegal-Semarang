<div class="container-content">
    <row class="mt-3">
        <div class="col-md-6">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <button class="btn button liked likebtn" data="<?= $otomotif['id_thread'] ?>" title="Like"> Like </button>

                        </div>
                        <div class="col-sm-6">
                            <button class="btn button  disliked dislikebtn" data="<?= $otomotif['id_thread'] ?>" title="Dislike">Dislike</button>
                        </div>
                    </div>
            ...
            <form method="POST" action="<?= base_url('otomotif/kirimKomen/') . $otomotif['id_thread']; ?>">
            ...
            <div class="card mt-1">
                <div class="card-body">
                    ...
                    <?php foreach ($komentar as $komen) : ?>
                        ...
                        <?php if ($komen['id_kategori'] != $otomotif['id_kategori']) : ?>
                        <?php else : ?>
                            <?php if ($komen['id_thread'] == $otomotif['id_thread']) : ?>
                                ...
                                <?php else : ?>
                                    <a href="<?= base_url(); ?>otomotif/hapusKomen/<?= $komen['id_komentar']; ?>" class="badge badge-danger float-right" onclick="return confirm('Yakin?');">Hapus</a>

                                    <a href="<?= base_url(); ?>otomotif/ubah/<?= $komen['id_komentar']; ?>" class="badge badge-warning float-right">Ubah</a>
                                <?php endif; ?>
        ...
        <a href="<?= base_url(); ?>otomotif" class="btn btn-primary ml-3 mt-3">Kembali</a>
    </row>
</div>
...
<script>
    $('.liked').click(function() {
        ...
        if (btn.hasClass('unlikebtn')) {
            $.ajax({
                url: '<?= base_url('otomotif/unlike/') ?>' + id_thread,
        ...
        if (btn.hasClass('likebtn')) {
            $.ajax({
                url: '<?= base_url('otomotif/like/') ?>' + id_thread,
                success: function(result) {
    ...
    $('.disliked').click(function() {
        ...
        if (btn.hasClass('undislikebtn')) {
            $.ajax({
                url: '<?= base_url('otomotif/undislike/') ?>' + id_thread,
                success: function(result) {
        ...
        if (btn.hasClass('dislikebtn')) {
            $.ajax({
                url: '<?= base_url('otomotif/dislike/') ?>' + id_thread,
                success: function(result) {
